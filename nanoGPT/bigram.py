...
parser.add_argument(
    ...
    type=int,
    default=16,
    help="how many independent sequences will we process in parallel (default: 4)",
)
parser.add_argument(
    "--block_size",
    type=int,
    default=32,
    help="what is the maximum context length for predictions? (default: 8)",
)
parser.add_argument(
    "--max_iters",
    type=int,
    default=5000,
    help="how many training iterations do we want? (default: 3000)",
)
parser.add_argument(
    "--eval_interval",
    type=int,
    default=100,
    help="how often do we evaluate the loss on train and val? (default: 300)",
)
parser.add_argument(
    "--learning_rate",
    type=float,
    default=1e-3,
    help="what is the learning rate for the optimizer? (default: 0.001)",
)
parser.add_argument(
    "--eval_iters",
    type=int,
    default=200,
    help="how many batches we average the loss over for train and val (default: 200)",
)
parser.add_argument(
    "--n_embed",
    type=int,
    default=64,
    help="how many dimensions do we want to embed the tokens in? (default: 32)",
)
parser.add_argument(
    "--n_layer",
    type=int,
    default=4,
    help="how many layers of transformer blocks (default: 3)",
)
parser.add_argument(
    "--n_head",
    type=int,
    default=4,
    help="how many heads do we want to use? (default: 4)",
)
parser.add_argument(
    "--dropout",
    type=float,
    default=0.0,
    help="what dropout probability do we want to use? (default: 0.1)",
)
...
class BigramLanguageModel(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits[
                :, -1, :
            ]  # Becomes (B, C) (get the last time step for each sequence)
...
for i in range(max_iters):
    ...
    if i % eval_interval == 0:
        print(
            f" step {i} | train loss: {estimate_loss()['train']:.4f} | val loss: {estimate_loss()['val']:.4f}"
        )
